<?php
	...
	*/
	...
	$query = "SELECT o.order_id, i.name, o.dateAndTime, o.status, ohi.quantity, i.category 
	...
	$stmt->bind_result($order_id, $name, $dateAndTime, $status, $quantity, $category);

	$rows = array();
	while($stmt->fetch()){
		$row = array("order_id"=>$order_id, "name"=>$name, "dateAndTime" => $dateAndTime, "status" => $status, "quantity"=>$quantity, "category"=>$category); 
		$rows[] = $row;
	}
